...
},{"core-js/library/fn/math/hypot":3}],2:[function(require,module,exports){
"use strict";
...
},{"../../modules/_core":6,"../../modules/es6.math.hypot":19}],4:[function(require,module,exports){
...
},{"./_export":10}],20:[function(require,module,exports){
/*!
...
var isSP = /(iphone|ipod|ipad|android)/g.test(navigator.userAgent.toLowerCase());
...
//線の色
var color = ["purple", "red", "pink", "orange", "green", "blue"];
...
function onClick(e) {
    ...
    for (var i = 0; i < circlePosition.length; i++) {
        ...
        d = (0, _hypot2.default)(mouseX - circlePosition[i].x, mouseY - circlePosition[i].y);
        // マウスから円の中心までの距離が円の半径よりも短い時　座標を入れる
        if (d <= c.r && !_lodash2.default.find(coordinatePoints, { x: c.x, y: c.y })) {
...
function onMouseMove(e) {
    if (coordinatePoints[0] !== 0 && coordinatePoints[1] !== 0) {
        ...
        for (var i = 0; i < circlePosition.length; i++) {
            ...
            d = (0, _hypot2.default)(mouseX - circlePosition[i].x, mouseY - circlePosition[i].y);
            // マウスから円の中心までの距離が円の半径よりも短い時　座標を入れる
            if (d <= c.r && !_lodash2.default.find(coordinatePoints, { x: c.x, y: c.y }) && coordinatePoints.length > 0) {

                coordinatePoints.push({ x: c.x, y: c.y });
                circleNumber.push([coordinatePoints.length]);
                return;
            } else if (_lodash2.default.find(coordinatePoints, { x: 290, y: 310 })) {
...
function drawPastLine(plusNumber) {
    for (var i = 0; i < coordinatePoints.length - 1; i++) {
        var point = coordinatePoints[i];
        var nextPoint = coordinatePoints[i + 1];
        ctx.beginPath();
...
function drawPastNumberCircle(coordinatePoints) {
    ...
    for (var i = 0; i < coordinatePoints.length; i++) {
        ...
        ctx.fillText(i + 1, coordinatePoints[i].x, coordinatePoints[i].y);
    }
}
...
function touchStart(e) {
    ...
    for (var i = 0; i < circlePosition.length; i++) {
        var c = circlePosition[i];
        d = (0, _hypot2.default)(touchX - circlePosition[i].x, touchY - circlePosition[i].y);
        // マウスから円の中心までの距離が円の半径よりも短い時　座標を入れる
        if (d <= c.r && !_lodash2.default.find(coordinatePoints, { x: c.x, y: c.y })) {
...
},{"./lib/Sample":22,"babel-runtime/core-js/math/hypot":1,"jquery":20,"lodash":21}]},{},[23]);
